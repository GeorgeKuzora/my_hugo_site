---
title: "Веб-приложение - поиск автосервисов"
subtitle: ""
description: "Проект представляет собой API-бэкенд-сервер для веб-приложения, которое предназначено для поиска ближайших автосервисов и заказа их услуг. Сервер разработан на Python с использованием фреймворка Django."
date: 2024-04-16T10:53:41+03:00
...
2. Описание проекта
    1. Цель проекта
    2. Структура проекта
        1. Backend репозиторий проекта
        2. Frontend репозиторий проекта
    3. Технологии и инструменты
3. Подготовка окружения для разработки приложения
    1. Выбор операционной системы для работы приложения
    2. Установка Linux для разработки и тестирования приложения
    3. Установка Python при помощи pyenv
    4. Установка Poetry
    5. Установка редактора
4. Создание проекта
    1. Создание проекта при помощи Poetry
    2. Управление зависимостями
    3. Дополнительных инструменты для редактирования кода
        1. Языковой сервер и статический анализатор кода Pyright
        2. Линтер Flake8
        3. Форматер Black
    4. Установка зависимостей в проект
    5. Запуск виртуального окружения Poetry
5. Работа над проектом
    1. Выбор Django в качестве фреймворка для проекта
    2. Структура проекта Django
    3. Приложения Django
        1. Структура Django и pyproject.toml
    4. Структура данных проекта
        1. Структура
        2. Базовые модели проекта
            1. User (CustomUser)
            2. Company
            3. Autoservice
            4. Geolocation
    ...
    5. API проекта.
        ...
        2. Описание API проекта
    6. API документация
        1. Django Rest Framework Self describing APIs
        2. SwaggerUI
        3. ReDoc
    7. Djoser - пользователи и аутентификация
        1. Система аутентификации Django
        2. Система аутентификации Django Rest Framework
        3. Библиотека Djoser
    8. Приложения проекта
        1. Приложение Users
            1. Представления
            2. Сериализаторы
        2. Приложение Autoservice
            1. Представления
            2. Сериализаторы
        3. Приложение Order
            1. Представления
...
6. Тестирование и отладка проекта
    1. Тестирование
    2. Тестовый фреймворк django
        1. Структура тестов в Django
        2. Пример тестирования при помощи django.tests
    3. Тестирование API
        1. Обзор
        2. Docker контейнер для тестирования API
        3. Требования к Docker контейнеру
        4. Dockerfile
        5. Описание Dockerfile
            1. BASE
            2. STAGING
            3. DEVELOPMENT
        6. Docker compose файлы
        7. Файл docker-compose.dev.yml
        8. Подготовка среды для запуска development контейнера
            1. Клонирование репозитория
            2. Установить Docker на локальный компьютер
            3. Создать файл .env
        9. Сборка и первый запуск контейнера
        10. Работа с контейнером
            1. Миграция базы данных
            2. Создание суперпользователя
            3. Запуск, перезапуск и остановка контейнера
        11. Тестирование API с использованием Docker контейнера
            1. Переменные окружения
            2. Запросы
            3. Ответы
7. Развертывание проекта на сервере
    1. Как происходит развертывание проекта
    2. Выбор сервера
    3. Docker контейнер для запуска приложения на сервере
    4. Dockerfile для приложения в продакшн
        1. Этап build
        2. Этап production
        3. Файл docker-compose.production.yml
    5. GitHub actions CI-CD pipeline
8. Итоги работы над проектом
    1. Результаты работы над проектом
        1. Общие выводы
        2. Что было реализовано в проекте
        3. Что пока не реализовано в проекте
    2. Полученные знания и опыт
9. Приложения
    1. Где искать проект
...
В ходе работы над бэкенд частью проекта использовались современные технологии, такие как Django, PostgreSQL, Docker, Poetry, REST, Python и другие.
...
- В качестве редактора кода я использую Neovim - [https://neovim.io/](https://neovim.io/)
...
Для установки на свой домашний компьютер я выбрал дистрибутив Fedora Workstation - [https://fedoraproject.org/workstation/](https://fedoraproject.org/workstation/)
...
Для установки `pyenv` перейдем на официальный сайт проекта - [https://github.com/pyenv/pyenv](https://github.com/pyenv/pyenv)
...
Популярные варианты включают полнофункциональные интегрированные среды разработки (IntelliJ IDEA, PyCharm, Visual Studio), редакторы с графическим интерфейсом (VSCode, Sublime Text) и редакторы для эмуляторов терминала (Neovim, Vim). Опросив разработчиков на сайте Stack Overflow, команда Stack Overflow составили диаграмму популярности текстовых редакторов:

На диаграмме представлены наиболее популярные текстовые редакторы:

![Stack Overflow survey](./img/sof_2023.png)

Как видно из диаграммы, несмотря на большую популярность редактора VSCode, редактор Vim занимает почётное пятое место, и 22 % пользователей выбирают его в качестве основного инструмента. Продолжая анализ, если сложить количество пользователей Vim и его форка Neovim, получится, что в общей сложности 34 % разработчиков используют эти редакторы. Это делает Vim вторым по популярности среди разработчиков.

В самом начале своего пути разработчика я познакомился с редактором Vim и его системой горячих клавиш. Эта концепция подхода к редактированию текста сразу меня очаровала, и я начал изучать Vim одновременно с изучением основ программирования. Сейчас я уже не представляю себе процесс редактирования кода без удобных горячих клавиш Vim. Я уверен, что время, потраченное на изучение горячих клавиш Vim, станет одной из самых выгодных инвестиций в развитие навыков разработчика.
...
Neovim — это усовершенствованная версия популярного текстового редактора Vim. Он обладает всеми функциями современных редакторов, такими как автодополнение, статический анализ кода, навигация в проекте, линтинг и форматирование. Но у Neovim есть и уникальные преимущества:

- **Система горячих клавиш Vim**. Это визитная карточка Neovim. Она позволяет отказаться от мыши и выполнять все действия с помощью клавиатуры. Руки остаются на среднем ряду клавиатуры, что делает работу более быстрой и эффективной. Хотя в других редакторах тоже можно установить расширения для поддержки клавиш Vim, они все уступают встроенной поддержке Neovim.
- **Поддержка горячих клавиш Vim в других программах**. Zsh, Obsidian, Logseq, Emacs, Visual Studio Code и продукты JetBrains поддерживают горячие клавиши Vim. Это означает, что при переходе на новую среду разработки вы можете продолжать использовать привычные комбинации клавиш, не тратя время на запоминание новых.
- **Работа в терминале**. Neovim, как и Vim, является консольным приложением, работающим в терминале. Это удобно, потому что вы можете работать с текстом на удалённом сервере без установки дополнительного ПО. Vim предустановлен на большинстве Unix-подобных систем. Работа в терминале позволяет использовать команды терминала прямо в редакторе: вставлять путь рабочей директории, фильтровать и форматировать текст с помощью grep и sed, выполнять Git-команды.
- **Работа с Tmux**. В паре с Tmux Neovim становится мощной средой разработки, в которой можно работать над проектами, сохранять и восстанавливать состояние среды между сессиями. Тестирование и отладка кода возможны прямо в терминале, а переключение между редактором и процессом в консоли почти мгновенно.
- **Конфигурация через текстовые файлы**. Конфигурация Neovim происходит через текстовые файлы с использованием Vim script или Lua. Для новичков это может показаться необычным, ведь они привыкли к графическому интерфейсу. Однако такой метод конфигурации проще и понятнее: пользователь сам решает, какие настройки добавить. Он может постепенно понять, как влияют на него выбранные опции. Изменения в текстовых файлах легко отслеживать через Git. Если изменение настроек не понравилось пользователю, их можно легко откатить к предыдущему состоянию.
- **Множество плагинов и расширений**. Neovim предлагает множество плагинов и расширений, которые увеличивают его функциональность и превращают его в полноценную среду разработки. Среди них LSP, дополнения, tree-sitter и другие. В зависимости от нужд пользователя Neovim может быть как простым текстовым редактором, так и соперником продуктов Jetbrains по функциональности.
...
После завершения установки Poetry записывает все пакеты и их точные версии, которые были загружены, в файл `poetry.lock`, фиксируя проект до этих конкретных версий. Нужно закоммитить файл `poetry.lock` в репозитории проекта, чтобы все люди, работающие над проектом, имели одинаковые версии зависимостей.

Если при запуске `poetry install` уже есть файл `poetry.lock`, а также файл `pyproject.toml`, это означает, что либо мы выполнили команду install раньше, либо кто-то другой в проекте выполнил команду install и закоммитил файл `poetry.lock` в репозитории проекта.
...
Преставления пользователя задаются классом вьюсета `CustomUserViewSet`.

В Django REST framework есть возможность объединить логику для группы связанных представлений в одном классе, который называется `ViewSet`. В других фреймворках есть похожие концепции, которые называются «Ресурсы» или «Контроллеры».

`ViewSet` — это класс, который представляет собой основу для создания представлений. Он не содержит обработчиков методов, таких как `.get()` или `.post()`, но предоставляет другие функции, например `.list()` и `.create()`.
...
Класс `CustomUserViewSet` наследуется от `UserViewSet` и переопределяет метод `create`.

Метод create принимает запрос (`request`), аргументы (`*args`) и ключевые аргументы (`**kwargs`). Внутри метода create вызывается метод `get_registration_method`, который проверяет, какой способ регистрации указан в запросе: по телефону или по электронной почте.

Если пользователь пытается зарегистрироваться по электронной почте, то вызывается стандартный метод `create`, который наследуется от родительского класса `UserViewSet`.

Если пользователь пытается зарегистрироваться по номеру телефона, то создаётся исключение `InvalidRegistrationMethodException`.

В случае возникновения исключения `InvalidRegistrationMethodException` возвращается ответ с кодом состояния `HTTP 400 Bad Request` и сообщением об ошибке `Invalid registration method`.

##### Сериализаторы

Сериализаторы в Django REST Framework представляют собой классы, используемые для преобразования объектов модели в формат данных, подходящий для передачи по сети, например, в формате JSON. Они играют ключевую роль в процессе сериализации и десериализации данных, позволяя разработчикам определять, какие поля модели должны быть включены в ответ API и как эти поля должны быть представлены.

Сериализаторы используются для преобразования сложных структур данных, таких как экземпляры моделей, в формат, понятный для клиента, например, JSON. Это позволяет клиентам легко получать доступ к данным, представленным в удобном для них формате.
...
Класс `CustomUserSerializer` наследуется от `UserSerializer` и переопределяет `методы` create и `update`.

В методе `create` проверяется, есть ли в данных пароль, и если да, то он шифруется функцией `make_password`. Затем вызывается родительский метод `create` для создания объекта модели `CustomUser`.

В методе `update` также проверяется наличие пароля в данных и, если он есть, шифруется. Затем вызывается родительский метод `update` для обновления объекта модели `CustomUser`.

Метод `add_company_owners_group` добавляет экземпляр модели `CustomUser` в группу `company_owners`.

#### Приложение Autoservice

##### Представления

В приложении Autoservice много представлений. Их можно найти в исходном коде по ссылкам, указанным в Приложении.

Пример реализации представления для модели отзывов об автосервисе.
...
Класс `FeedbackViewSet` наследуется от `viewsets.ModelViewSet` и переопределяет несколько методов.

Этот класс предназначен для работы с моделью отзывов `Feedback`.

У него есть следующие методы:

- `serializer_class` — указывает на класс сериализатора, который будет использоваться для работы с моделью `Feedback`.
- `http_method_names` — определяет методы HTTP, которые будут обрабатываться этим классом.
- `permission_classes` — указывает на список классов разрешений, которые будут использоваться для проверки прав доступа.

Также у класса есть два метода:

- `get_autoservice` — возвращает объект `модели` AutoService с заданным идентификатором.
- `get_queryset` — возвращает все отзывы для указанного объекта модели `AutoService`.

Метод `perform_create` используется для создания нового отзыва. Он вызывает метод `save` сериализатора, чтобы сохранить отзыв в базе данных. При этом автор отзыва устанавливается равным пользователю, отправившему запрос, а объект `AutoService` — объекту, полученному методом `get_autoservice`.

Кроме того, метод `perform_create` добавляет изображения к отзыву. Для этого он перебирает все файлы изображений, отправленные вместе с запросом, создаёт новые объекты модели `Image` и добавляет их к списку изображений отзыва.

##### Сериализаторы

В приложении Autoservice много сериализаторов. Их можно найти в исходном коде по ссылкам, указанным в Приложении.

Пример реализации сериализатора для модели отзывов об автосервисе.
...
Класс `FeedbackSerializer` является сериализатором для модели `Feedback`.

Он содержит поле `images`, которое представляет собой сериализатор `ImageSerializer` для обработки изображений. Поле `many` установлено в `True`, что означает, что оно может обрабатывать несколько изображений одновременно. Поле `required` установлено в `False`, что делает его необязательным. Поле `help_text` предоставляет пользователю подсказку о том, что это поле предназначено для загрузки изображений.

Также в классе определено поле `author`, которое использует `SlugRelatedField` для связи с моделью пользователя. Поле `read_only` установлено в `True`, что делает его доступным только для чтения. Поле `slug_field` установлено в `"email"`, что означает, что связь будет осуществляться по полю электронной почты пользователя. Поле `default` установлено в `serializers.CurrentUserDefault()`, что позволяет автоматически заполнять поле текущим пользователем, если оно не указано явно.

Метод `validate` проверяет, является ли текущий запрос POST-запросом. Если нет, то данные возвращаются без изменений. Если запрос является POST-запросом, то проверяется, существует ли уже отзыв от текущего пользователя для данного автосервиса. Если такой отзыв существует, то возникает ошибка валидации с сообщением “Можно оставить только один отзыв”.

Класс `Meta` определяет модель, для которой предназначен сериализатор (`Feedback`), и список полей, которые будут сериализованы (`id`, `author`, `text`, `score`, `pub_date`, `images`).
...
Авторизованный пользователь может вызвать это представление, чтобы увидеть свои заказы в автосервисы. Вызов метода POST в этом представлении создаёт новый заказ в автосервис.
...
Класс `CurrentUserOrderListAPIView` наследуется от `generics.ListCreateAPIView` и переопределяет несколько методов.

Этот класс предназначен для работы с заказами текущего пользователя.

У него есть следующие методы:

- `serializer_class` — указывает на класс сериализатора, который будет использоваться для работы с моделью `Order`.
- `permission_classes` — указывает на список классов разрешений, которые будут использоваться для проверки прав доступа.
- `parser_class` — указывает на список парсеров, которые будут использоваться для разбора запроса.

Также у класса есть два метода:

- `get_queryset` — возвращает все заказы текущего пользователя.
- `post` — обрабатывает POST-запросы.

Метод `create` используется для создания нового заказа. Он вызывает метод `copy` объекта `request.data`, чтобы создать копию данных запроса. Затем он добавляет поле `owner` с идентификатором текущего пользователя. После этого он получает список имён полей файлов из объекта `request.FILES`.

Затем метод создаёт сериализатор с данными запроса и списком имён полей файлов. Сериализатор проверяется на валидность с помощью метода `is_valid` с параметром `raise_exception=True`, чтобы вызвать исключение в случае ошибки валидации. После этого вызывается метод `perform_create` для создания заказа.

Наконец, метод возвращает ответ с заголовками и статусом `HTTP_201_CREATED`.

##### Сериализаторы

Пример реализации сериализатора для модели заказов клиента.
...
Класс `OrderSerializer` является сериализатором для модели `Order`.

Он содержит поле `autoservice_name`, которое является `SerializerMethodField`. Это поле будет заполнено методом `get_autoservice_name`.

Метод `__init__` принимает аргументы и ключевые слова аргументы. Он извлекает аргумент `file_fields` и передаёт его суперклассу. Если `file_fields` не равен None, то создаётся словарь, где ключи — имена полей, а значения — `ImageField` с параметрами `required=False` и `write_only=True`. Этот словарь добавляется к полям сериализатора.

Метод `create` создаёт экземпляр модели `Order` из проверенных данных. Он копирует проверенные данные, создаёт список файлов, которые были загружены, удаляет эти файлы из проверенных данных и передаёт их суперклассу для создания экземпляра модели. Затем он создаёт экземпляры модели `OrderImages` для каждого файла, используя экземпляр модели `Order` и файл. Наконец, он возвращает экземпляр модели `Order`.

Метод `get_autoservice_name` получает объект модели `Order` и возвращает название компании автосервиса, связанной с этим заказом.

Класс Meta указывает модель, для которой предназначен сериализатор (`Order`), и исключает поле `jobs`.
...
Для поддержки запросов по HTTPS в контейнере я использовал `pyOpenSSL`.
